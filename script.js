...
const getPosition = async () => {
  const successHandler = async (position) => {
    try {
      ...
      let response = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&units=metric&appid=b275f0cfb1a5bf36b58fbf288d72591b`
      );
      let result = await response.json();
  ...
  const errorHandler = (error) => {
    // console.log(error)
    cityName.innerHTML =
      "Please allow your location access or search manually !";
  };
